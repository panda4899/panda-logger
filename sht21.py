...
if __name__ == "__main__":
    try:
        with SHT21(1) as sht21:
            ...
            #myfile.write(strftime("%m-%e-%y %H:%M"))  # time
            #myfile.write(",")
            #myfile.write(str(round(temp, 2)))  # temp
            #myfile.write(",")
            #myfile.write(str(round(humid, 2)))  # humidity
            #myfile.write("\n")
            #myfile.close()
        #with open("log.csv", "a") as myfile:
            #myfile.write("time") # time
        #with open("log.csv", "a") as myfile:
            #myfile.write(sht21.read_temperature()) # temp
        #with open("log.csv", "a") as myfile:
            #myfile.write(sht21.read_humidity()) # humidity
    except IOError, e:
        print e
